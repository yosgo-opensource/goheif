        ...
        if (shdr->chroma_weight_flag[l][i])
          for (int j = 0; j < 2; j++)
          {
            ...
            if (vlc < -4 * sps->WpOffsetHalfRangeC ||
                vlc > 4 * sps->WpOffsetHalfRangeC - 1)
              return false;

            vlc = Clip3(-sps->WpOffsetHalfRangeC,
                        sps->WpOffsetHalfRangeC - 1,
                        (sps->WpOffsetHalfRangeC + vlc - ((sps->WpOffsetHalfRangeC * shdr->ChromaWeight[l][i][j]) >> shdr->ChromaLog2WeightDenom)));
  ...
  *continueDecoding = false;
  ...
  if (!dependent_slice_segment_flag)
  {
        ...
        ctx->get_nal_unit_type() != NAL_UNIT_IDR_N_LP)
    {
      ...
      else
      {
        ...
        if (nBits > 0)
          short_term_ref_pic_set_idx = get_bits(br, nBits);
        else
          short_term_ref_pic_set_idx = 0;
  ...
  if (pps->slice_segment_header_extension_present_flag)
  {
    ...
    if (slice_segment_header_extension_length == UVLC_ERROR ||
        slice_segment_header_extension_length > 1000)
  ...
  if (!dependent_slice_segment_flag)
  {
        ...
        nal_unit_type != NAL_UNIT_IDR_N_LP)
    {
      ...
      else
      {
        int nBits = ceil_log2(sps->num_short_term_ref_pic_sets());
        if (nBits > 0)
          out.write_bits(short_term_ref_pic_set_idx, nBits);
        else
        {
          assert(short_term_ref_pic_set_idx == 0);
        }
...
void slice_segment_header::compute_derived_values(const pic_parameter_set *pps)
{
  ...
  case SLICE_TYPE_I:
    ...
    break;
  case SLICE_TYPE_P:
    initType = cabac_init_flag + 1;
    break;
  case SLICE_TYPE_B:
    initType = 2 - cabac_init_flag;
    break;
  }
                                ...
                                int x0, int y0, int ctDepth)
{
  ...
  if (availableL && tctx->img->get_ctDepth(x0 - 1, y0) > ctDepth)
    condL = 1;
  if (availableA && tctx->img->get_ctDepth(x0, y0 - 1) > ctDepth)
    condA = 1;
                               ...
                               int x0, int y0, int ctDepth)
{
  ...
  if (availableL && tctx->img->get_cu_skip_flag(x0 - 1, y0))
    condL = 1;
  if (availableA && tctx->img->get_cu_skip_flag(x0, y0 - 1))
    condA = 1;
          ...
          for (int yC = 0; yC < (1 << log2w); yC++)
            for (int xC = 0; xC < (1 << log2w); xC++)
            {
              ...
              else
              {
                ...
                {
                case 0:
                  sigCtx = (xP + yP >= 3) ? 0 : (xP + yP > 0) ? 1
                                                              : 2;
                  break;
                case 1:
                  sigCtx = (yP == 0) ? 2 : (yP == 1) ? 1
                                                     : 0;
                  break;
                case 2:
                  sigCtx = (xP == 0) ? 2 : (xP == 1) ? 1
                                                     : 0;
                  break;
                default:
                  sigCtx = 2;
                        ...
                        int xC, int yC, int xN, int yN)
{
  ...
  if (xN >= img->get_sps().pic_width_in_luma_samples)
  {
    return 0;
  }
  ...
  for (int i = lastSubBlock; i >= 0; i--)
  {
    ...
    */
    ...
    if (nCoefficients)
    {
      ...
      if (cIdx == 0)
        predModeIntra = img->get_IntraPredMode(x0, y0);
      else
        predModeIntra = img->get_IntraPredModeC(x0, y0);
                        ...
                        int cbf_luma, int cbf_cb, int cbf_cr)
{
  ...
  if (cbf_luma)
  {
    if ((err = residual_coding(tctx, x0, y0, log2TrafoSize, 0)) != DE265_OK)
      return err;
  }
  ...
  if (log2TrafoSize > 2 || ChromaArrayType == CHROMA_444)
  {
...

    {
      if (cbf_cb & 1)
      {
        if ((err = residual_coding(tctx, x0, y0, log2TrafoSizeC, 1)) != DE265_OK)
          return err;
      }
...

    {
      if (cbf_cr & 1)
      {
        if ((err = residual_coding(tctx, x0, y0, log2TrafoSizeC, 2)) != DE265_OK)
          return err;
      }
